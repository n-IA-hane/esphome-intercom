...
`esphome.<slug>_start_call`, which lets `voip_stack.call` originate from a
selected physical ESP device. Full runtime-controller profiles instead combine
the low-level `ha_integration.yaml` entity surface with `ha_api_runtime.yaml`.
