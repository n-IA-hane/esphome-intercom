...
class FrontendCardContractTest(unittest.TestCase):
    ...
    def test_browser_media_controls_are_session_owned_and_available_in_call(self) -> None:
        ...
        self.assertIn('keypadLabelIcon.setAttribute("icon", "mdi:dialpad")', self.view_source)
        self.assertIn(".video-active:not(.settings-open) .runtime-controls .keypad-label { display: none; }", self.view_source)
        self.assertIn("padding-right: calc(2 * clamp(50px, 16vw, 58px));", self.view_source)
        self.assertIn("els.keypadLabel.textContent = keypadButtonLabel", self.source)
