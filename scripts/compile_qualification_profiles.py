...
from scripts.candidate_lock import DEFAULT_SOURCES, load_lock  # noqa: E402
...
@contextmanager
def _locked_workspaces(candidate_path: Path):
    """Materialize the exact clean revisions recorded by a candidate lock."""

    candidate = load_lock(candidate_path)
    sources = json.loads(DEFAULT_SOURCES.read_text(encoding="utf-8"))["repositories"]
    repositories = candidate.get("repositories")
    if not isinstance(repositories, dict) or set(repositories) != set(sources):
        raise RuntimeError("candidate repository set does not match qualification sources")
    if any(bool(item.get("dirty")) for item in repositories.values()):
        raise RuntimeError("firmware compilation requires a clean candidate lock")

    with tempfile.TemporaryDirectory(prefix="voip-candidate-") as temporary:
        parent = Path(temporary)
        worktrees: list[tuple[Path, Path]] = []
        try:
            for name, source in sources.items():
                repository = (ROOT / source["path"]).resolve()
                target = parent / name
                commit = str(repositories[name]["commit"])
                subprocess.run(
                ...
                worktrees.append((repository, target))
            staged_root = parent / "esphome-intercom"
            subprocess.run(
                [sys.executable, str(staged_root / "scripts/yaml_paths.sh"), "local"],
                cwd=staged_root,
                check=True,
            )
            yield staged_root, candidate
        finally:
            for repository, target in reversed(worktrees):
                subprocess.run(
...
def main() -> int:
    ...
    with _locked_workspaces(args.candidate_lock.resolve()) as (staged_root, candidate):
        paths = [staged_root / profile.path for profile in profiles]
        with _test_secrets(paths) as placeholder_directories:
            for profile, path in zip(profiles, paths, strict=True):
                subprocess.run(
                    [sys.executable, "-m", "esphome", "compile", str(path)],
                    cwd=staged_root,
                    check=True,
                )
                ...
                record = next(
                    (
                        item
                        for item in build_records(staged_root)
                        if any(
                            staged_root / str(artifact.get("path") or "") == source
                ...
                for artifact in record.get("artifacts", []):
                    artifact_source = staged_root / str(artifact.get("path") or "")
