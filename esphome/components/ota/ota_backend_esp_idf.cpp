...
OTAResponseTypes IDFOTABackend::end() {
  // IDF validates the full image in esp_ota_end() and again when selecting
  // the boot partition. Keep the existing scoped watchdog protection used
  // for erase, restoring the ordinary timeout on every exit from this phase.
  const size_t validation_size = this->partition_ != nullptr ? this->partition_->size : 0;
  watchdog::WatchdogManager watchdog(15000 + (validation_size >> 10));
  if (this->md5_set_) {
    this->md5_.calculate();
